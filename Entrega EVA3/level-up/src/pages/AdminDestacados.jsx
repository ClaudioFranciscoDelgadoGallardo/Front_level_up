...
export default function AdminDestacados() {
  ...
  const cargarProductos = async () => {
    try {
      ...
      console.log('📦 Productos destacados (Admin) cargados:', productosActivos.length);
    } catch (error) {
        ...
        <div className="admin-card-body">
          ...
          {productos.length === 0 && !cargando && (
            <div className="alert alert-warning mt-3 mb-0">
              No hay productos activos registrados. <Link to="/admin/productos/nuevo" className="alert-link">Crear producto</Link>
            </div>
          )}
          {cargando && (
            <div className="alert alert-info mt-3 mb-0">
